...
```{r fulllmodelImportance, echo=FALSE, fig.width = 6, fig.height =5, fig.cap = "Figure 4: Relative importance of predictors in full random forest model"}
...
```{r resultsDetailed, echo=FALSE, fig.cap="Figure 5.  Comparison of Moodel Accuracy and Kappa", fig.height=4, fig.width=5}
...
```{r summaryVals, results=FALSE, include = FALSE, eval=FALSE}
## Remove the first 'header' columns
#dd <- d[,-(1:7)]
set.seed(1234)
inTrain = createDataPartition(d.summary$classe, p = 3/4)[[1]]
training = d.summary[ inTrain,]
testing = d.summary[-inTrain,]

# Cart
mod.cart <- train(classe~., data=training, method="rpart")
pred.cart <- predict(mod.cart,testing)
confusionMatrix(testing$classe,pred.cart)
...
# gbm
set.seed(1234)
mod.gbm <- train(classe~., data = training, method = "gbm", verbose=FALSE)
predtrain.gbm <- predict(mod.gbm,training)
pred.gbm <- predict(mod.gbm,testing)
confusionMatrix(testing$classe,pred.gbm)


## Stacking models
stackem <- data.frame(classe=training$classe, gbm=predtrain.gbm, 
                      rf=predtrain.rf)
modelStack <- train(classe ~ ., data=stackem, method="rf")
testStack <- data.frame(classe=testing$classe,gbm=pred.gbm, 
                      rf=pred.rf)
testFinal <- predict(modelStack,testStack)
confusionMatrix(testing$classe,testFinal)
```

```{r consolidate1, include=FALSE, eval=FALSE}
## Consolidate results for assessment
results <- resamples(list(CART=mod.cart,GBM = mod.gbm,RF=mod.rf, Stack=modelStack))
summary(results)$statistics[1]
scales <- list(x=list(relation="free"), y=list(relation="free"))
bwplot(results, scales=scales)
```


``` {r loopPlots3, include=FALSE, eval=FALSE}
g <- ggplot(d, aes(magnet_forearm_x,magnet_forearm_z, colour=classe))
g <- g + geom_point(alpha=0.2)
g <- g + facet_wrap(~user_name,ncol=3)
print(g)


g <- ggplot(d, aes(magnet_belt_y,magnet_belt_z, colour=classe))
g <- g + geom_point(alpha=0.2)
g <- g + facet_wrap(~user_name,ncol=3)
print(g)


g <- ggplot(d, aes(accel_belt_y,accel_belt_z, colour=classe))
g <- g + geom_point(alpha=0.2)
g <- g + facet_wrap(~user_name,ncol=3)
print(g)

g <- ggplot(d, aes(magnet_arm_x,magnet_arm_z, colour=user_name))
g <- g + geom_point(alpha=0.2)
g <- g + facet_wrap(~classe,ncol=3)
print(g)

g <- ggplot(d, aes(magnet_dumbbell_x,magnet_dumbbell_y, colour=classe))
g <- g + geom_point(alpha=0.2)
g <- g + facet_wrap(~user_name,ncol=3)
print(g)
...
``` {r loopPlots2, echo=FALSE, include=FALSE, eval=FALSE}
g <- ggplot(d, aes(accel_arm_x,accel_arm_y, colour=classe))
g <- g + geom_point(alpha=0.2)
g <- g + facet_wrap(~user_name,ncol=3)
print(g)

g <- ggplot(d, aes(accel_arm_x,accel_arm_z, colour=classe))
g <- g + geom_point(alpha=0.2)
g <- g + facet_wrap(~user_name,ncol=3)
print(g)

g <- ggplot(d, aes(accel_arm_z,accel_arm_y, colour=classe))
g <- g + geom_point(alpha=0.2)
g <- g + facet_wrap(~user_name,ncol=3)
print(g)
